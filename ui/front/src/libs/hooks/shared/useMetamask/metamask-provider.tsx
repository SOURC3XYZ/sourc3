...
function subsribeToAccountsChanged(dispatch: (action: Action) => void) {
  const { ethereum } = window as WindowInstanceWithEthereum;
  const onAccountsChanged = (accounts: string[]) => dispatch({ type: 'metaMaskAccountsChanged', payload: accounts });
...
function subscribeToChainChanged(dispatch: (action: Action) => void) {
  const { ethereum } = window as WindowInstanceWithEthereum;
  const onChainChanged = (chainId: string) => dispatch({ type: 'metaMaskChainChanged', payload: chainId });

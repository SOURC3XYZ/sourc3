...
{
	git::InitialParams params;

	Env::GenerateKernel(nullptr, git::InitialParams::METHOD, &params, sizeof(params), nullptr, 0, nullptr, 0, "Create git contract", 0);
}

void On_action_destroy_contract(const ContractID& cid)
{
	Env::GenerateKernel(&cid, 1, nullptr, 0, nullptr, 0, nullptr, 0, "Destroy git contract", 0);
}

void On_action_view_contracts(const ContractID& unused)
{
	EnumAndDumpContracts(git::s_SID);
}
...
static int git_error__check_version(const void *structure, unsigned int expected_max, const char *name)
{
//	unsigned int actual;
//
//	if (!structure)

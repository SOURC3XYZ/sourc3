...
struct Control {
    ...
    struct ExplicitUpgrade : public Base {
        static const uint8_t s_Type = 1;
        ExplicitUpgrade() : Base(s_Type) {
        }

        // induce the scheduled upgrade. Without the need to 'hijack' another
        // method. doesn't need to be signed (i.e. any user can invoke)
    };

    struct OnUpgraded : public Base {
    ...
    struct Signed : public Base {
        Signed(uint8_t nType) : Base(nType) {
        }
        uint32_t
            m_ApproveMask;  // bitmask of approvers. Each must sign this call
    };

    struct ScheduleUpgrade : public Signed {
    ...
    struct ReplaceAdmin : public Signed {
    ...
    struct SetApprovers : public Signed {

...
BEAM_EXPORT void Method_2(const Ctl::Base& r_) {
    ...
    switch (r_.m_Type) {
        case Ctl::ExplicitUpgrade::s_Type: {
            NextVersion::Key nvk;
            uint32_t nVal =
                Env::LoadVar(&nvk, sizeof(nvk), nullptr, 0, KeyTag::Internal);
    ...
    switch (r_.m_Type) {
        case Ctl::ScheduleUpgrade::s_Type: {
            ...
            Env::Halt_if(r.m_Next.m_hTarget <
                         Env::get_Height() + stg.m_hMinUpgradeDelay);

            NextVersion::Key nvk;
            Env::SaveVar(&nvk, sizeof(nvk), &r.m_Next,
                         sizeof(r.m_Next) + r.m_SizeShader, KeyTag::Internal);
        }

 ...
 public:
  ...
  int DoFetch(const vector<string_view>& args) {
    ...
    std::vector<GitObject> objects;
    {
      ...
      auto res = wallet_client_.InvokeWallet("role=user,action=repo_get_meta");
    ...
    while (!object_hashes.empty()) {
      ...
      if (it == objects.end()) {
        // cerr << "No object data for " << objectToReceive << ", possibly
        // submodule" << endl;
  ...
  int DoPush(const vector<string_view>& args) {
    ...
    {
      auto it =
          std::unique(objs.begin(), objs.end(), [](auto&& left, auto& right) {
            return left.oid == right.oid;
          });
    ...
    {
      auto progress =
          MakeProgress("Uploading objects to IPFS", collector.m_objects.size());
      size_t i = 0;
      for (auto& obj : collector.m_objects) {
        if (obj.selected) {
          continue;
        ...
        if (obj.GetSize() > kIpfsAddressSize) {
          auto res =
              wallet_client_.SaveObjectToIPFS(obj.GetData(), obj.GetSize());
...

    {
      ...
      collector.Serialize([&](const auto& buf, size_t done) {
        ...
        if (!buf.empty()) {
          ...
          //    reinterpret_cast<const GitObject*>(p + 1); for (uint32_t i = 0;
          //    i < p->objects_number; ++i)
          //    {
          ...
          //        GitObject*>(reinterpret_cast<const uint8_t*>(cur) + s);
          //    }
          //    std::cerr << std::endl;
          //}
    ...
    }
    {
      ...
      auto res =
          wallet_client_.WaitForCompletion([&](size_t d, const auto& error) {
            if (progress) {
              if (error.empty()) {

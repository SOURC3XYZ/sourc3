...

        {
            ...
            for (auto& obj : collector.m_objects) {
                ...
                if (obj.GetSize() > kIpfsAddressSize) {
                    ...
                    if (options_.compress) {
                        auto compressed =
                            Compress(obj.GetData(), obj.GetSize());
                        res = wallet_client_.SaveObjectToIPFS(
                            compressed.data(), compressed.size());
                    } else {
                        res = wallet_client_.SaveObjectToIPFS(obj.GetData(),
                                                              obj.GetSize());
                    }
...

        {
            ...
            collector.Serialize([&](const auto& buf, size_t done) {
                ...
                if (!buf.empty()) {
                    ...
                    //    std::cerr << std::endl;
                    //}
                    ByteBuffer data = options_.compress ? Compress(buf) : buf;
                    str_data = ToHex(data.data(), data.size());
                }

...
struct Manager {
    ...
    struct SettingsPlus : public Settings {
...
#define Upgradable3_deploy(macro)                               \
    macro(Height, hUpgradeDelay) macro(uint32_t, nMinApprovers) \
        macro(uint32_t, bSkipVerifyVer)

#define Upgradable3_multisig_args(macro) \
    macro(uint32_t, iSender) macro(uint32_t, approve_mask)

#define Upgradable3_schedule_upgrade(macro)       \
    macro(ContractID, cid) macro(Height, hTarget) \
        macro(uint32_t, bSkipVerifyVer) Upgradable3_multisig_args(macro)

#define Upgradable3_replace_admin(macro)                             \
    macro(ContractID, cid) macro(uint32_t, iAdmin) macro(PubKey, pk) \
        Upgradable3_multisig_args(macro)

#define Upgradable3_set_min_approvers(macro)       \
    macro(ContractID, cid) macro(uint32_t, newVal) \
        Upgradable3_multisig_args(macro)

    static uint32_t get_ChargeDeploy() {
        return Env::Cost::CallFar + Env::Cost::SaveVar_For(sizeof(Settings)) +
...
#pragma pack(pop)
        ...
        uint32_t m_Charge =
            Env::Cost::CallFar + Env::Cost::LoadVar_For(sizeof(Settings)) +
        ...
        void InvokeKrn(const Secp_scalar_data& kSig, Secp_scalar_data* pE) {
            Env::GenerateKernelAdvanced(
                m_pCid, m_iMethod, m_pArg, m_nArg, nullptr, 0, m_pPks, m_nPks,
                m_szComment, m_Charge, m_Msg2.m_hMin, m_Msg2.m_hMin + s_dh,
                m_Msg2.m_pkKrnBlind, m_Msg2.m_pkFullNonce, kSig,
                s_iSlotKrnBlind, s_iSlotKrnNonce, pE);
        }
            ...
            if (iAdmin == iSender) {
                ...
                for (uint32_t iPeer = 0; iPeer < stg.s_AdminsMax; iPeer++) {
                    ...
                    if (iPeer == iAdmin)
                        ...
                        auto& cc = pPeers[iPeer];
                        Env::WriteStr("waiting for co-signer nonce",
                                      Stream::Out);
        ...
        static bool Perform_ScheduleUpgrade(const VerInfoBase& vi,
            ...
            uint32_t nSizeArgs =
                sizeof(Method::Control::ScheduleUpgrade) + nShaderSize;
            auto* pArgs =
                (Method::Control::ScheduleUpgrade*)Env::Heap_Alloc(nSizeArgs);
            auto& arg = *pArgs;
        ...
        static void Perform_ReplaceAdmin(const ContractID& cid,
                                         const Env::KeyID& kid, uint32_t iAdmin,
                                         const PubKey& pk) {
            ...
            msp.m_Charge += Env::Cost::SaveVar_For(sizeof(Settings));

            msp.Perform();
        }

        static void Perform_SetApprovers(const ContractID& cid,
                                         const Env::KeyID& kid,
                                         uint32_t newVal) {
        ...
        static void Perform_ExplicitUpgrade(const ContractID& cid,
            ...
            nChargeExtra +=
                Env::Cost::CallFar * 2 + Env::Cost::LoadVar +
                Env::Cost::LoadVar_For(nVal) + Env::Cost::HeapOp * 2 +
                Env::Cost::MemOpPerByte * nVal + Env::Cost::SaveVar +
                Env::Cost::UpdateShader_For(nVal - sizeof(nv)) +
                Env::Cost::Cycle * 1000;  // other stuff

            Method::Control::ExplicitUpgrade arg;
            Env::GenerateKernel(&cid, Method::Control::s_iMethod, &arg,
                                sizeof(arg), nullptr, 0, nullptr, 0,
                                "Upgradable3 explicit upgrade", nChargeExtra);
        }
    };
            ...
            for (uint32_t iVer = 0; iVer < m_Versions; iVer++) {
                ...
                for (wlk.Enum(m_pSid[iVer]); wlk.MoveNext();) {
...

                    {
                        ...
                        for (Env::LogReader rVer(kVer, kVer);
                             rVer.MoveNext_T(kVer, sid);) {
                            Env::DocGroup gr3("");
...

                    {
                        SettingsPlus stg;
                        if (stg.Read(cid)) {
                            Env::DocAddNum("min_upgrade_delay",
                                           stg.m_hMinUpgradeDelay);
...

                    {
                        ...
                        if (r.MoveNext(&k, nKey, nullptr, nVal, 0) &&
                            (nVal >= sizeof(NextVersion))) {
...
#define ARG_NAME_PREFIX "admin-"
            char szBuf
                [_countof(ARG_NAME_PREFIX) +
                 Utils::String::Decimal::Digits<Settings::s_AdminsMax>::N] =
                    ARG_NAME_PREFIX;

            uint32_t iFree = arg.s_AdminsMax;

            for (uint32_t i = 0; i < Settings::s_AdminsMax; i++) {
                Utils::String::Decimal::Print(
                    szBuf + _countof(ARG_NAME_PREFIX) - 1, i);
...
#undef ARG_NAME_PREFIX
        ...
        bool ScheduleUpgrade(const ContractID& cid, const Env::KeyID& kid,
                             Height hTarget) const {
            return Manager::MultiSigRitual::Perform_ScheduleUpgrade(
                *this, cid, kid, hTarget);
        }
    };
};
